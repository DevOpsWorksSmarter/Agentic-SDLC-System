...
class TestOrchestrator:
    def test_all_tasks_reach_terminal_state(self):
        ...
        assert non_terminal == [], (
            f"Non-terminal tasks: {[t.name for t in non_terminal]}"
        )
    ...
    def test_rejected_task_skips_dependents(self):
        ...
        WorkflowOrchestrator(auto_approve=False, approval_callback=reject_arch).run(
            state
        )
...
class TestArtifacts:
    ...
    def test_validation_checks_all_pass(self):
        ...
        blocking_failures = [
            c for c in state.validation.checks if c.get("blocking") and not c["passed"]
        ]
